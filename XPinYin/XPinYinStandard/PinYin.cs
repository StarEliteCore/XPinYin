 ...
 *
 * 设计思路：
 * 将汉字按拼音分组后建立一个字符串数组（见PinYinCode.codes），然后使用程序
 * 将PinYinCode.codes中每一个汉字通过其编码值使用散列函数：
 *
 *     f(x) = x % PyCode.codes.Length
 *     g(f(x)) = pos(x)
 *
 * 其中, pos(x)为字符x所属字符串所在的PinYinCode.codes的数组下标, 然后散列到同PinYinCode.codes长度相同长度的一个散列表中PinYinHash.hashes。
 * 当检索一个汉字的拼音时，首先从PinYinHash.hashes中获取和对应的PinYinCode.codes中数组下标，然后从对应字符串查找，
